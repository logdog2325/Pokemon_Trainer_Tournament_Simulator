...
let OPPONENTS = [], TEAMS = {};
let MY_PASTE = null;    // the team you pilot (null = the engine's built-in default team)
const SIM = new SimClient('../sim/worker.js');   // the in-browser engine (no server)
async function boot() {
	await SIM.ready;
	const data = await SIM.meta();
	SPRITES = data.sprites || {};
	OPPONENTS = data.opponents || [];
	TEAMS = data.teams || {};
	// hand-off from the builder: ?opp=<name>&team=<base64 pokepaste>  (Battle Lab "Play ▶")
	const qs = new URLSearchParams(location.search);
	let handoff = null;
	if (qs.get('team')) { try { handoff = decodeURIComponent(escape(atob(qs.get('team').replace(/-/g, '+').replace(/_/g, '/')))); } catch { handoff = null; } }
...
function renderSetup(handoff, preOpp) {
	...
	s.innerHTML = `<h2>Set up your battle</h2>
		<div class="setup"><label>You pilot</label>
			<select id="pilotSel">${opts}</select>
			<textarea id="pilotPaste" class="hidden" placeholder="paste a Showdown export / Champions team…"></textarea></div>
		<h2 style="margin-top:14px">Opponent <span class="muted">(AI-piloted)</span></h2>
		<div id="oppList"></div>`;
	const sel = $('#pilotSel'), ta = $('#pilotPaste');
	sel.onchange = () => {
		const v = sel.value;
	...
	OPPONENTS.forEach(name => {
		const b = el('button', 'opp' + (name === preOpp ? ' rnd' : ''), name);
		b.onclick = () => startBattle(name);
		list.appendChild(b);
	});
}
function startBattle(opponent) {
	battleId = SIM.startBattle(opponent, MY_PASTE, (line) => enqueue(line));
