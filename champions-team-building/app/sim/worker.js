...
self.onmessage = async (e) => {
  ...
  try {
    if (m.type === 'meta') {
      ...
      const best = await CS.optimize(m.paste, m.opponent, m.n, (done, total) => self.postMessage({ rid: m.rid, type: 'progress', done, total }));
      self.postMessage({ rid: m.rid, type: 'result', best });
